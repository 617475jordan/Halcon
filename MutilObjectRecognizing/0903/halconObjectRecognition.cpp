...
void halconObjectRecognition::run(HObject ho_SearchImage, Mat m_matImg)
{
	...
	for (int j = 0; j < hv_vecHtRow.size(); ++j)
	{
		...
		if (hv_Score.Length() > 0)
		{
			...
			if (m_dAngle > 0 && m_dAngle < PI/2)
			{
				m_matImg = drawArrow(m_matImg, Point(m_dCurrent_X, m_dCurrent_Y), Point(m_dCurrent_X - m_iLen*sin(m_dAngle), m_dCurrent_Y - m_iLen*cos(m_dAngle)),  m_dAngle);
			}
			else if (m_dAngle > PI / 2 && m_dAngle < PI )
			{
				m_matImg = drawArrow(m_matImg, Point(m_dCurrent_X, m_dCurrent_Y), Point(m_dCurrent_X - m_iLen*sin(m_dAngle), m_dCurrent_Y + m_iLen*cos(m_dAngle)),  m_dAngle  );

			}
			else if (m_dAngle > PI  && m_dAngle < 3*PI / 2)
			{
				m_matImg = drawArrow(m_matImg, Point(m_dCurrent_X, m_dCurrent_Y), Point(m_dCurrent_X + m_iLen*sin(m_dAngle), m_dCurrent_Y + m_iLen*cos(m_dAngle)),  m_dAngle );

			}
			else if (m_dAngle > 3 * PI / 2 && m_dAngle < 2*PI)
			{
				m_matImg = drawArrow(m_matImg, Point(m_dCurrent_X, m_dCurrent_Y), Point(m_dCurrent_X + m_iLen*sin(m_dAngle), m_dCurrent_Y - m_iLen*cos(m_dAngle)),  m_dAngle  );

			}
			else if (m_dAngle > -PI / 2 && m_dAngle < 0)
			{
				m_matImg = drawArrow(m_matImg, Point(m_dCurrent_X, m_dCurrent_Y), Point(m_dCurrent_X + m_iLen*sin(m_dAngle), m_dCurrent_Y - m_iLen*cos(m_dAngle)), m_dAngle);
			...
			{
				m_matImg = drawArrow(m_matImg, Point(m_dCurrent_X, m_dCurrent_Y), Point(m_dCurrent_X, m_dCurrent_Y - m_iLen), m_dAngle);

			}
			else if (m_dAngle == PI / 2)
			{
				m_matImg = drawArrow(m_matImg, Point(m_dCurrent_X, m_dCurrent_Y), Point(m_dCurrent_X +m_iLen, m_dCurrent_Y),  m_dAngle);
			...
			else if (m_dAngle == PI * 3 / 2 || m_dAngle == -PI / 2)
			{
				m_matImg = drawArrow(m_matImg, Point(m_dCurrent_X, m_dCurrent_Y), Point(m_dCurrent_X + m_iLen, m_dCurrent_Y),  m_dAngle);
...
Mat halconObjectRecognition::drawArrow(cv::Mat& img, cv::Point pStart, cv::Point pEnd,  double angle)
{
	...
	if (angle > PI / 2 && angle < PI * 3 / 2)
	{
		...
		arrow.x = pEnd.x + m_iLineLen * cos(angle - m_dalpha + PI / 2);

		arrow.y = pStart.y + m_iLineLen * sin(angle - m_dalpha + PI / 2);

		line(img, pStart, arrow, m_scalarColor, m_iThickness, m_iLineType);

		arrow.x = pStart.x + m_iLineLen * cos(angle + m_dalpha + PI / 2);
	...
	else
	{
		...
		arrow.y = pEnd.y + m_iLineLen * sin(angle + m_dalpha + PI / 2);
